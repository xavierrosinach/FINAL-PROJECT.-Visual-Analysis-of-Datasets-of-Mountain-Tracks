...
def extract_zip(zone, extract_path):
    ...
    raw_zip_path = f'../../Data/Raw-Data/{zone}.zip'        # Define the path of the zip file
    provisional_path = f'../../Data/Raw-Data/provisional_{zone}'    # Provisional path to extract the zip


    if not os.path.exists(raw_zip_path):  # If the zip file does not exist, break
        raise FileNotFoundError(f"Error: The file {zone}.zip does not exist or is not in the correct directory.")   # Raise an error
    
    with zipfile.ZipFile(raw_zip_path, 'r') as zip_file:        # Extract the zip file to the provisional path
        zip_file.extractall(provisional_path)
    
    os.makedirs(extract_path, exist_ok=True)    # Ensure extract_path exists
...
def create_network(zone, network_path):
    ...
    os.makedirs(network_path)    # Create the directory
...
def discard_coordinates(zone, coordinates_df, percentage_inside_bounds=0.5, min_coordinates=100, max_distance=300, min_total_distance=1000):
    ...
    return True, 0, total_distance      # If any error, return true, and we can proceed with the track processing
...
def output_process(file_path, out_df, out_df_path, zone, info, fmm_result, total_distance, k, r, e, no_osm_df, no_osm_df_path):
    ...
    out_df = pd.concat([out_df, pd.DataFrame({'track_id':[info['track']], 'zone':[zone], 'url':info['url'], 'user':info['user'], 'title':[info['title']],
                                              'date_up':[info['date_up']], 'activity_type':[info['activity_type']], 'activity_type_name':[info['activity_name']], 
                                              'difficulty':[info['difficulty']], 'distance':[np.round(total_distance,2)],  'k':[k], 'radius':[r], 'gps_error':[e],
    ...
    coordinates_df["MatchedLongitude"], coordinates_df["MatchedLatitude"] = zip(*coords)
    ...
    coordinates_df[['TransformedLongitude', 'TransformedLatitude']] = coordinates_df.apply(
        lambda row: transformer.transform(row['MatchedLongitude'], row['MatchedLatitude']), 
...
def process_all_tracks(zone, raw_path, output_path, output_tracks_path, model):
    ...
    if os.path.exists(out_df_path):
        ...
        out_df = pd.DataFrame(columns=['track_id','zone','url','user','title','date_up','activity_type','activity_type_name','difficulty','distance','k','radius','gps_error','mean_point_error'])
    ...
    i = 1   # To iterate (for the print)

    # Iterate through all files
    for file in os.listdir(raw_path):

        track_id = file.split('.')[0]           # Obtain the track id
        ...
        if int(track_id) not in processed_files:            # Check if the file is already processed or not
            ...
            if valid_file:              # If the file is valid, we will process with the matching
...
def main_processing(zone):
    ...
    if not os.path.exists(network_path):    # Create the network if it is not created
        create_network(zone, network_path)

    network = Network(os.path.join(network_path, 'edges.shp'), "fid", "u", "v")     # Network of the zone with FMM
